...
class EmbossingPlateApiService {
  ...
  Future<EmbossingPlatePageDto> fetchEmbossingPlates({
    ...
    if (payload is Map<String, dynamic>) {
      ...
      final list = results is List
          ? results
                .whereType<Map>()
                .map(
                  (item) => EmbossingPlateDto.fromJson(
    ...
    if (payload is List) {
      final list = payload
          ...
          .map(
            (item) =>
                EmbossingPlateDto.fromJson(Map<String, dynamic>.from(item)),
          )
          .toList();
      return EmbossingPlatePageDto(
        items: list,
    ...
    return const EmbossingPlatePageDto(
      items: [],
      total: 0,
      page: 1,
      pageSize: 20,
  ...
  Future<EmbossingPlateImage> uploadImage(
    int plateId,
