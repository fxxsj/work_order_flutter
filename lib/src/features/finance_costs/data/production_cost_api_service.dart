...
class ProductionCostApiService {
  ...
  Future<ProductionCostPageDto> fetchCosts({
    ...
    final trimmedPeriodStart = periodStart?.trim();
    if (trimmedPeriodStart != null && trimmedPeriodStart.isNotEmpty) {
      params['period_start'] = trimmedPeriodStart;
    }
    final trimmedPeriodEnd = periodEnd?.trim();
    if (trimmedPeriodEnd != null && trimmedPeriodEnd.isNotEmpty) {
      params['period_end'] = trimmedPeriodEnd;
    }
    ...
    if (payload is Map<String, dynamic>) {
      ...
      final list = results is List
          ? results
                .whereType<Map>()
                .map(
                  (item) => ProductionCostDto.fromJson(
  ...
  Future<Map<String, dynamic>> calculateMaterial(int id) async {
    final response = await _client.post(
      '/production-costs/$id/calculate_material/',
    );
  ...
  Future<Map<String, dynamic>> calculateTotal(int id) async {
    final response = await _client.post(
      '/production-costs/$id/calculate_total/',
    );
    return _mapFromResponse(response.data);
  }
