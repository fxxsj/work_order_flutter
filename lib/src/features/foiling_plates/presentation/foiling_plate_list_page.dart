...
class _FoilingPlateListViewState extends State<_FoilingPlateListView> {
  ...
  final Set<_FoilingPlateColumn> _visibleColumns = {
    _FoilingPlateColumn.code,
    _FoilingPlateColumn.name,
    _FoilingPlateColumn.type,
    _FoilingPlateColumn.size,
    _FoilingPlateColumn.material,
    _FoilingPlateColumn.thickness,
    _FoilingPlateColumn.confirmed,
    _FoilingPlateColumn.products,
    _FoilingPlateColumn.notes,
    _FoilingPlateColumn.createdAt,
    _FoilingPlateColumn.actions,
  };
  ...
  void _openColumnsMenu(BuildContext context) {
    ...
    showMenu<_FoilingPlateColumn>(
      context: menuContext,
      position: position,
      items: _FoilingPlateColumn.optionalValues.map((value) {
        final checked = _visibleColumns.contains(value);
        return CheckedPopupMenuItem<_FoilingPlateColumn>(
          value: value,
          checked: checked,
  ...
  List<DataColumn> _buildColumns() {
    ...
    if (_visibleColumns.contains(_FoilingPlateColumn.code)) {
      columns.add(const DataColumn(label: Text('烫金版编码')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.name)) {
      columns.add(const DataColumn(label: Text('烫金版名称')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.type)) {
      columns.add(const DataColumn(label: Text('类型')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.size)) {
      columns.add(const DataColumn(label: Text('尺寸')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.material)) {
      columns.add(const DataColumn(label: Text('材质')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.thickness)) {
      columns.add(const DataColumn(label: Text('厚度')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.confirmed)) {
      columns.add(const DataColumn(label: Text('确认状态')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.products)) {
      columns.add(const DataColumn(label: Text('包含产品')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.notes)) {
      columns.add(const DataColumn(label: Text('备注')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.createdAt)) {
      columns.add(const DataColumn(label: Text('创建时间')));
    }
    if (_visibleColumns.contains(_FoilingPlateColumn.actions)) {
      columns.add(const DataColumn(label: Text('操作')));
    }
    return columns;
  }

  List<DataRow> _buildRows(BuildContext context, FoilingPlateViewModel viewModel, List<FoilingPlate> plates) {
    ...
    return plates.map((plate) {
      ...
      if (_visibleColumns.contains(_FoilingPlateColumn.type)) {
        cells.add(DataCell(Text(_displayText(_typeLabel(plate.foilingType)))));
      }
      if (_visibleColumns.contains(_FoilingPlateColumn.size)) {
  ...
  static Widget _productCell(List<FoilingPlateProduct> products) {
    if (products.isEmpty) {
...
enum _FoilingPlateColumn {
  ...
  static const List<_FoilingPlateColumn> optionalValues = [
    _FoilingPlateColumn.type,
    _FoilingPlateColumn.size,
    _FoilingPlateColumn.material,
    _FoilingPlateColumn.thickness,
  ...
  String get label {
    switch (this) {
      case _FoilingPlateColumn.code:
        return '烫金版编码';
      case _FoilingPlateColumn.name:
        return '烫金版名称';
      case _FoilingPlateColumn.type:
        return '类型';
      case _FoilingPlateColumn.size:
        return '尺寸';
      case _FoilingPlateColumn.material:
        return '材质';
      case _FoilingPlateColumn.thickness:
        return '厚度';
      case _FoilingPlateColumn.confirmed:
        return '确认状态';
      case _FoilingPlateColumn.products:
        return '包含产品';
      case _FoilingPlateColumn.notes:
        return '备注';
      case _FoilingPlateColumn.createdAt:
        return '创建时间';
      case _FoilingPlateColumn.actions:
        return '操作';
    }
  }
}
...
class _PaginationBar extends StatelessWidget {
  ...
  final FoilingPlateViewModel viewModel;
...
class _FoilingPlateListTile extends StatelessWidget {
  const _FoilingPlateListTile({
    required this.plate,
    this.onEdit,
    this.onDelete,
    this.onConfirm,
  ...
  Widget build(BuildContext context) {
    ...
    final tile = ListTile(
      ...
      title: Text(
        plate.name.isNotEmpty ? plate.name : _FoilingPlateListViewState._emptyCellText,
...
class _EmptyState extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    return Container(
      ...
      child: Column(
        children: [
          Icon(Icons.auto_fix_high_outlined, color: theme.colorScheme.primary, size: _iconSize),
          const SizedBox(height: _FoilingPlateListViewState._spacingSm),
          Text(_FoilingPlateListViewState._emptyText, style: theme.textTheme.bodyMedium),
        ],
      ),
    );
  }
      ...
      child: Column(
        children: [
          ...
          OutlinedButton.icon(
            ...
            label: const Text(_FoilingPlateListViewState._retryText),
          ),
        ],
      ),
    );
