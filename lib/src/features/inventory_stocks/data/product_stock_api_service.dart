  ...
  Future<ProductStockPageDto> fetchProductStocks({
    ...
    if (payload is List) {
      final list = payload
          .whereType<Map>()
          .map(
            (item) => ProductStockDto.fromJson(Map<String, dynamic>.from(item)),
          )
          .toList();
      return ProductStockPageDto(
        items: list,
